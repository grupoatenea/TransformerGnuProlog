...
public class GNUPrologMaxAggExpression implements GNUPrologExpression {
	...
	public String toArithmeticForm() {
		...
		String gnuMaxExpression = new StringBuilder().append( "max(" ).append( GNUPrologTransformer.transform(expressions.get(0)).toArithmeticForm() ) .append( ", " ).append( GNUPrologTransformer.transform(expressions.get(1)).toArithmeticForm() ).append( ")" ).toString();
		int i = 2;
		while ( i <= expressions.size() - 1 ) {
			gnuMaxExpression = new StringBuilder().append( "max(" ) .append( gnuMaxExpression ).append( ", " ).append( GNUPrologTransformer.transform(expressions.get(i)).toArithmeticForm() ).append( ")" ).toString();
			i++;
		}
