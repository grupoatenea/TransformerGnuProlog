...
public class GNUPrologSumAggExpression implements GNUPrologExpression {
	...
	public String toArithmeticForm() {
		...
		String list = GNUPrologTransformer.transform( new ParenthesesExpression( expressions.get(0) ) ).toArithmeticForm();
		int i = 1;
		while ( i <= expressions.size()-1) {
			list = list.concat(" + " + GNUPrologTransformer.transform( new ParenthesesExpression( expressions.get(i) ) ).toArithmeticForm());
			i++;
		}
		return list;
	}
